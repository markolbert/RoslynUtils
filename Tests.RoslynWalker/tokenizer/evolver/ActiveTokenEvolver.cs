...
namespace Tests.RoslynWalker
{
    public class ActiveTokenEvolver : IActiveTokenEvolver
    {
        ...
        private readonly Type[] _convTypes =
        {
            ...
            typeof(TokenSpawnInfo)
        };

        public ActiveTokenEvolver()
        {
            _evolvers.Add( new TokenLeadingConverter( "[", TokenType.Attribute ) );
            _evolvers.Add( new TokenLeadingConverter( "(", TokenType.ArgumentList ) );
            ...
            _evolvers.Add( new TokenLeadingConverter( "<", TokenType.TypeArgument ) );
            ...
            _evolvers.Add( new TokenTextMatchesConverter("get", TokenType.Property, TokenRelativePosition.Parent)  );
            _evolvers.Add( new TokenTextMatchesConverter("set", TokenType.Property, TokenRelativePosition.Parent)  );
            _evolvers.Add( new TokenTextMatchesConverter("init", TokenType.Property, TokenRelativePosition.Parent)  );
            ...
            //_matchers.Add( new MethodTokenConverter() );

            _evolvers.Add( new CloseFiltered( 
                "\r", 
                true, 
                TokenizerExtensions.IgnoresEOLCharacters) );
            ...
            _evolvers.Add( new CloseFiltered( "*/", true, t => t != TokenType.MultiLineComment ) );
            _evolvers.Add( new CloseFiltered( false, t => true ).AddClosers( "\t", ":", " ", ")", "," ) );
            _evolvers.Add( new CloseFiltered( true,
                    t => true,
                    s => s.Tokens.Any( t => t.Type.IsRequiresBracesType() ) )
                .AddClosers( "{", "}" ) );

            _evolvers.Add( new TokenTrailingModifier( "//", t=> t == TokenType.SingleLineComment ) );
            _evolvers.Add( new TokenTrailingModifier( "\r" ) );
            _evolvers.Add( new TokenTrailingModifier( "\n" ) );
            _evolvers.Add( new TokenTrailingModifier( "\t", " " ) );
            _evolvers.Add( new TokenTrailingModifier( "  ", " " ) );

            //_matchers.Add( new TokenSpawner("/*", TokenType.MultiLineComment)  );
            //_matchers.Add( new TokenSpawner("[", TokenType.PropertyIndexer)  );
            //_matchers.Add( new TokenSpawner("//", TokenType.SingleLineComment)  );
            //_matchers.Add( new TokenSpawner("<", TokenType.TypeArgument)  );
        }

        public bool EvolveActiveToken( Token.TokenCollection tokenCollection, out TokenEvolutionInfo? result )
        {
            result = null;
        ...
        private bool RunEvolvers( IEnumerable<ITokenEvolver> evolvers, Token.TokenCollection tokenCollection, out TokenEvolutionInfo? result )
        {
            result = null;
