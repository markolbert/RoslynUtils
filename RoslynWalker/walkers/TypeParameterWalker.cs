...
{
    [ RoslynProcessor( typeof(NamedTypeWalker) ) ]
    public class TypeParameterWalker : SyntaxWalker<ITypeParameterSymbol>
    {
        private static readonly List<SyntaxKind> _ignoredNodeKinds = new List<SyntaxKind>();

        static TypeParameterWalker()
        {
        ...
        protected override bool NodeReferencesSymbol( SyntaxNode node, 
            ...
            out ITypeParameterSymbol? result )
        {
            result = null;
            ...
            if( !context.GetSymbol<ITypeParameterSymbol>( node, out var typeSymbol ) )
            {
                Logger.Verbose<string, SyntaxKind>("{0}: no ITypeParameterSymbol found for node of kind {1}",
